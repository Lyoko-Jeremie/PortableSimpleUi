...
一个简易GUI框架，支持快速构建一个简单的界面，适合于一些诸如游戏Mod的小工具。

针对只能在页面初始化后再加载的环境特别优化。

底层使用 ZoneJs + 脏检测 实现简易的双向数据绑定。

提供默认样式（css+scss），支持自定义样式。

支持样式隔离，组件样式默认使用 Shadow DOM 隔离，避免作为Mod使用时，库的样式与页面本身的冲突。
